...
import { getSupportVideosListAPI } from "@/apis/videos"
import { useRoute, onBeforeRouteUpdate } from "vue-router"   // 引入路由

const route = useRoute()                //调用路由
const VideosList = ref([])             //产品数据列表
const getVideosList = async(id = route.params.id) => {
    const res = await getSupportVideosListAPI(id)
    // SelfData.value = res.category[0]
    VideosList.value = res.data                // 产品列表数据
    // console.log(VideosChannel.value);
}


onMounted(() => getVideosList())
</script>
...
<div class="Video-panel">
    <div class="container">
        <div class="Video-display">
            <div class="item"  v-for="item in VideosList" :key="item.id">
                <div class="pic-box">
                ...
                <RouterLink class="title" :to="`/video/detail/`"><h4>{{ item.name }}</h4></RouterLink>
